# A Hybrid Modeling Approach for Clinical Decision-Making Under Uncertainty
# Overview
...
This study demonstrates that predictive performance and decision quality are fundamentally different objectives. Across the results, traditional machine learning models achieve reasonable accuracy and, in some cases, favorable error rates for unnecessary testing and missed useful testing. For example, Boosted Trees shows relatively low values on both metrics. However, these metrics alone do not capture whether decisions are appropriate, consistent, or aligned with clinical goals. They reflect outcomes of thresholded predictions rather than explicitly optimized decisions.

The hybrid model addresses this limitation by reframing the problem from prediction to decision-making. Instead of producing probabilities alone, it introduces decision-relevant quantities such as the probability that a test will change management, the risk of harm (e.g., unnecessary antibiotics), and the timing of information through turnaround time. These components are combined into action-specific utilities (order now, wait, do not order), allowing direct comparison of competing actions. As shown in Table 1 and the decision mechanism figures, the model incorporates value-of-information and time-dependent reasoning, which are essential in clinical settings but absent in standard machine learning approaches.

The results highlight this distinction. While the hybrid model does not minimize individual error components in isolation, it achieves a better balance between competing risks and demonstrates improved alignment with the optimal policy, as reflected in decision consistency. Figures 5 and 6 show that models such as SVM reduce missed useful orders by aggressively ordering tests, resulting in excessive unnecessary testing. In contrast, the hybrid model selectively orders tests when the expected benefit justifies the cost and delay. Table 3 further shows that extreme testing strategies do not improve downstream outcomes, emphasizing that decision quality cannot be inferred from prediction metrics alone.

Importantly, the hybrid model provides interpretability at both the system and patient levels. Table 4 illustrates how decisions are derived from explicit comparisons of utilities, incorporating uncertainty, timing, and outcome risks. This makes the decision process transparent and consistent with clinical reasoning, addressing a key limitation of black-box predictive models.

Figure 6 provides a direct comparison of decision-relevant errors between pure machine learning models and the hybrid model, highlighting an important trade-off. While models such as Boosted Trees appear to achieve lower unnecessary and missed useful order rates, these metrics alone do not reflect the quality of the underlying decision process. The hybrid model does not attempt to minimize either error independently; instead, it balances them based on expected utility, incorporating the probability that testing will change management, the risk of harm, and the effect of turnaround time.  The hybrid model intentionally avoids ordering tests when (1) Probability of changing management is low, (2) Turnaround time reduces value, or (3) Downstream harm outweighs benefit. This increases missed useful orders slightly, but reduces unnecessary interventions in a principled way. As a result, it may accept a slightly higher missed useful order rate in situations where the value of information is low or delayed, thereby avoiding unnecessary interventions. This behavior reflects a fundamentally different objective: optimizing decisions rather than predictions. Consequently, Figure 6 should be interpreted not as a comparison of classification performance, but as evidence of how decision-focused modeling produces more clinically aligned trade-offs between competing risks.


Despite these advantages, several limitations should be acknowledged. First, the framework relies on synthetic data, which enables controlled evaluation but may not fully capture real-world complexity. Second, the specification of utility functions introduces modeling assumptions that require careful calibration in practice. Third, the model depends on accurate estimation of intermediate quantities such as the probability of changing management, which may be challenging to learn from real data. Finally, while the framework improves decision consistency, its impact on real-world clinical outcomes remains to be validated.

Overall, this work represents a shift from descriptive to decision-science AI. The hybrid model is more powerful than traditional machine learning in that it directly optimizes decisions under uncertainty rather than relying on indirect thresholds applied to predictions. However, it should be viewed as a foundational step rather than a final solution.

# Conclusion

This study shows that improving predictive accuracy does not necessarily lead to better decisions. By integrating prediction, timing, uncertainty, and utility into a unified framework, the proposed approach moves beyond classification toward actionable decision support. While further validation and refinement are needed, this work represents a first step toward a new generation of AI systems that are designed not just to predict outcomes, but to guide decisions in complex, uncertain clinical environments.
